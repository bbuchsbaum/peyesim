...
CORAL is useful when the two domains differ by covariance structure rather
than a simple geometric warp. The example below creates matched four-element
density maps, then stretches the source domain along different latent axes.
Raw cosine similarity is already fairly high, but CORAL should recover the
known covariance alignment.

```{python}
def make_tiny_density(vec):
    """Create a small EyeDensity from a four-element vector."""
    return EyeDensity(
        x=np.arange(1, 3),
        y=np.arange(1, 3),
        z=np.asarray(vec, dtype=float).reshape(2, 2),
        sigma=50,
    )
...
n_coral = 80
base_mat = rng.normal(size=(n_coral, 4))
scale_mat = np.diag([2.0, 0.6, 1.7, 0.8])
source_mat = base_mat @ scale_mat

coral_ref_tab = pd.DataFrame({
    "id": np.arange(n_coral),
    "density": [make_tiny_density(row) for row in base_mat],
})

coral_source_tab = pd.DataFrame({
    "id": np.arange(n_coral),
    "density": [make_tiny_density(row) for row in source_mat],
})
```

```{python}
raw_coral = template_similarity(
    coral_ref_tab, coral_source_tab,
    match_on="id",
    permutations=0,
    method="cosine",
)
...
raw_coral_mean = raw_coral["eye_sim"].mean()
coral_positive_mean = coral_positive["eye_sim"].mean()

print(f"Raw mean similarity:   {raw_coral_mean:.3f}")
print(f"CORAL mean similarity: {coral_positive_mean:.3f}")
assert coral_positive_mean > raw_coral_mean, "CORAL should improve this positive control"
```
...
ax.bar(
    ["Raw", "CORAL"],
    [raw_coral_mean, coral_positive_mean],
    color=["#6b7280", "#2563eb"],
    alpha=0.85,
)
ax.set_ylim(0.85, 1.0)
ax.set_ylabel("Mean cosine similarity")
ax.set_title("CORAL recovers the simulated covariance shift")
plt.tight_layout()
plt.show()
```

This is still an in-sample diagnostic. It is useful because it proves the
transform hook can recover a known domain shift, but it should not be reported
as an out-of-sample effect estimate.
